...
/*   Updated: 2025/02/07 12:54:12 by vide-sou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_centralize(double *x, double *y, double z, t_section *section)
{
	...
	div_x = ((section->column - section->row) * cos(section->persp));
	div_y = ((section->column + section->row) * sin(section->persp) - z);
...
t_pixel_source	ft_distort_pixel(int x, int y, t_section *section)
{
	...
	inner_x += ft_per_pixel((x - y) * cos(section->persp), section);
	inner_y += ft_per_pixel((x + y) * sin(section->persp) - inner_z, section);
