...
/*   Updated: 2025/02/07 12:39:20 by vide-sou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_centralize(double *x, double *y, double z, t_section *section)
{
	...
	div_x = ((section->column - section->row) * cos(0.3));
	div_y = ((section->column + section->row) * sin(0.3) - z);
...
t_pixel_source	ft_distort_pixel(int x, int y, t_section *section)
{
	...
	inner_x += ft_per_pixel((x - y) * cos(0.3), section);
	inner_y += ft_per_pixel((x + y) * sin(0.3) - inner_z, section);
